...
While the multiplicity is a convenient and well defined observable to compare different collision systems, the underlying dynamics may be driven by other properties. In large collision systems, the energy density $\epsilon$ is often used to characterize the system and the expected effects. Figure~\ref{fig:energy_density} shows an estimate of the energy density for pp, p--Pb and Pb--Pb collisions using the Bjorken estimate. As input the $\langle\pT\rangle$~\cite{Abelev:2013bla,Acharya:2018njl} as well as the transverse overlap from IP-Glasma~\cite{McLerran:2013una} (for pp and p--Pb) and a Glauber MC~\cite{Loizides:2017ack} (for Pb--Pb) are used. The calculations for three different $\tau_0$ are shown. It should be noted that these assumptions can be challenged and other ways to calculate $\epsilon_0$ are available. Here the aim is only to show that the energy density depends on the system at a fixed multiplicity, and can easily reach large values in pp and p--Pb collisions.
...
\caption{Energy density as a function of $\dNdy$ under specific assumptions (see text), compared between pp, p--Pb, and Pb--Pb at $\tau = \unit[1]{fm/c}$. The line indicates the energy density reached in central Pb--Pb collisions ($\dNdy \approx 2000$).}
