...
\IfEq{\jobname}{\currfilebase}{\input{\main/smallsystems/commands}}{}
% until here
...
The main experimental task is future years is twofold: Firstly, a detailed characterization of the similarities and differences in the overlap region between pp, p--Pb and Pb--Pb collisions, and secondly, the study of the onset of the observed effects.\todo{extend and merge with discussion on why pp and pPb are needed}
...
While the multiplicity is a convenient and well defined observable to compare different collision systems, the underlying dynamics may be driven by other properties. In large collision systems, the initial energy density $\epsilon_0$ is often used to characterize the system and the expected effects. Figure~\ref{fig:energy_density} shows an estimate of the energy density for pp, p--Pb and Pb--Pb collisions using the Bjorken estimate. As input the $\langle\pT\rangle$~\cite{Abelev:2013bla} as well as the transverse overlap from IP-Glasma~\cite{McLerran:2013una} are used. The calculations for three different $\tau_0$ are shown. It should be noted that these assumptions can be challenged and other ways to calculate $\epsilon_0$ are available. Here the aim is only to show that the energy density depends on the system at a fixed multiplicity, and can easily reach large values in pp and p--Pb collisions.
...
\begin{figure}[ht]
\centering
\includegraphics[draft]{\main/smallsystems/img/energy_density.pdf}
\caption{Energy density as a function of $\dNdeta$ under some specific assumptions (see text), compared between pp, p--Pb, and Pb--Pb. Three different $\tau_0$ are assumed. The arrow on the right indicates the energy density reached in central Pb--Pb collisions ($\dNdeta \approx 2000$).}
...
\begin{figure}[ht]
\centering
\includegraphics[width=.45\linewidth]{\main/smallsystems/img/v2vspt_NCQ_deta1p05_sub_projection.pdf}
