...
The $\rho'$ state represents $\pi^+\pi^-\pi^+\pi^-$ states with masses in the range from 4$m_{\pi}$ up to 2.5~\UGeVcc. In absence of other guidance and the possibility of more complex resonance structure, the calculation is anchored to the STAR measurement of the $\pi^+\pi^-\pi^+\pi^-$ final state in Au--Au collisions at $\sqrtsnn=200$~\UGev~\cite{Abelev:2009aa} and uses STARlight to extrapolate in collision energy and collision system. 
