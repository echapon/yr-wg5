{ \small
\noindent \textbf{Coordinator}: Michael Winn (LAL, now DPhN, CEA/IRFU) %$^8$, now$^7$ 

\noindent \textbf{Contributors}:
A.~Angerami (LLNL Livermore), %$^{18}$, 
F.~Arleo (LLR/{\'E}cole Polytechnique), %$^5$, 
N.~Armesto (Universidade de Santiago de Compostela), %$^{17}$, 
R.~Bi (Massachusetts Institute of Technology), %.^{21}$ (MIT), %, 
{\'E}.~Chapon (CERN), %$^{23}$, 
Z.~Citron (Ben-Gurion University of the Negev), %$^{13}$,
...
M.~Klasen (M\"{u}nster University), %$^{11}$, 
S.~Klein (Lawrence Berkeley National Laboratory), %$^{18}$,
...
Y.-J.~Lee (Massachusetts Institute of Technology), %$^{21}$,  
M.~van~Leeuwen (Utrecht University/Nikhef), %$^{14}$,  
C.~Marquet (CPHT/{\'E}cole Polytechnique), %$^{4}$, 
...
H.~Paukkunen (University of Jyvaskyla, Helsinki Institute of Physics), % $^{2,3}$,  
