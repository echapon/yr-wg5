...
- higher statistics, better performance (through Muon Forward Tracker, for example)

- significantly improve measurements in extended kinematic coverage (in \pT) and for a larger variety of quarkonium states ($\psi(2S)$, $\Upsilon(3S)$?)

ADD ALSO kinametic coverage by experiments (with Figs.?) ...like:

Charmonia are measured in PbPb collisions by ALICE (\pT$>$0, for  $|y|<0.9$ and 
$2.5<y<4$), CMS (\pT$>$6.5 GeV/$c$, for  $|y|<1.2$, \pT$>$3.5 GeV/$c$, for  $1.2<|y|<2.4$ ?), and ATLAS (\pT$>$7 GeV/$c$, for  $|y|<2$).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Charmonia in PbPb collisions} % (Main contributor: Anton Andronic)}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

A remarkable discovery at the LHC was that the suppresson of J/$\psi$ is significantly reduced in comparison to lower energies \cite{} and that $\R_{AA}$ is enhanced towards lower \pT \cite{Abelev:2013ila,Adam:2016rdg}.
These observations were confirming predictions of (re)generation at the (crossover) phase boundary of QCD \cite{BraunMunzinger:2000px} or throughout the deconfined phase \cite{Thews:2000rj}.

Recently, the measurement of a significant elliptic flow coefficient $v_2$ of J/$\psi$ \cite{Acharya:2017tgv} brought further crucial support.
...
\end{center}
 \caption{Projected performance of elliptic flow coefficient $v_2$ of J/$\psi$ as a function of \pT~ for the centrality class 20--40\%, for $2.5<y<4$.}
\end{figure}
...
\end{center}
 \caption{Prompt J/$\psi$ RAA vs \pT~ ($|y|<2$) at high \pT~\cite{Aaboud:2018quy}}
\end{figure}
...
\end{center}
 \caption{Production ratio $\psi(2S)/(J/\psi$ vs. $N_{part}$ for $|y|<0.9$ (left) and $2.5<y<4$ (right). Model predictions for the transport approach and for statistical hadronization are included.}
\end{figure}

\begin{figure}
\begin{center}
 \includegraphics[width=0.6\textwidth]{\main/quarkonia/fig/theory/psi_dr_vitev.png}
\end{center}
 \caption{prompt psi(2S) (and J/psi, or ratio) vs. pT (at low and high pT) }
\end{figure}


Other states, for instance $\chi_c$, may be measured too, albeit not down to \pT=0.
