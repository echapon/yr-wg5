...
\subsubsection{\pPb collisions} % J.-P. Lansberg
...
\subsubsection{High-multiplicity pp collisions} %E. Ferreiro

The study of rare high-multiplicity events in proton-proton collisions at the LHC has allowed access to the dynamics of uncommon parton configurations in QCD. Striking results in such events, as the ridge-like structures of azimuthally-collimated and long-range in rapidity hadron correlations, lead to an exciting discussion on the relative importance of initial or final-state effects.
Systematic studies on quarkonium
production can play an important role in clarifying the underlying mechanisms of high-multiplicity events.
In particular, the correlation of open or hidden heavy-flavour yields with charged particles produced in hadronic collisions
can provide insight into their production mechanism and into the interplay between hard and soft mechanisms in particle production.

Hidden and open heavy-flavour production measurements as a function of the event activity were initiated during the LHC Runs 1-2  \cite{Abelev:2012rz,Chatrchyan:2013nza,Adam:2015ota}.  
The striking feature of the data is that the production yields of $D$-mesons and quarkonia in high multiplicity events are significantly enhanced relative to minimum bias events.
Specifically, the measurements of the self-normalized yields ({\it i.e.} the yield per event divided by the mean yield per event in minimum bias collisions) 
as a function of the self-normalized charged-particle multiplicity show an increase which is stronger than linear at the highest multiplicities for the central rapidity region.
%
Moreover, the compatibility between $D$-meson and $J/\psi$ results \cite{Abelev:2012rz,Adam:2015ota} suggests that this behaviour is most likely related to the production processes, and is not significantly influenced by hadronisation.
The measurement uncertainties prevent to conclude on a possible transverse momentum dependence.
When comparing $J/\psi$ preliminar results at $\sqrt{s}=13$ TeV \cite{Weber:2017hhm} to the previously ones obtained 
at $\sqrt{s} = 7$ TeV \cite{Abelev:2012rz}, no significant energy dependence is observed, {\it i.e.} the relative $J/\psi$ yields for events with identical relative multiplicities give similar results.

Two successful contenders remain today as possible explanations: 
On one hand, one can apply initial-state effects, within
the Color-Glass-Condensate (CGC) framework \cite{Ma:2018bax} or the percolation approach \cite{Ferreiro:2012fb,Ferreiro:2015gea}, which hint at possible saturation in high-multiplicity events in $pp$ collisions. 
...
On the other hand, final-state effects can be included through hydrodynamic evolution \cite{Werner:2013tya}, considering that the energy density reached in $pp$ collisions at 
LHC is high enough to apply such evolution.
...
The comparison of the above cited models to ALICE experimental data is shown in Figure~\ref{fig:fig}.

Runs 3-4 data, with the increased centre-of-mass energy of 14 TeV in $pp$ collisions and larger luminosities, can allow 
to reach higher multiplicities, in order to confirm the observed enhancement and its energy independence.
Moreover, differential studies in $p_T$ can be extremely useful to discriminate models.
For instance, in the percolation model, where colour interactions produce a dumping of the charged-particle multiplicities, the deviation from the linear behaviour is expected to be steeper for the harder $D$-meson or quarkonium yields.
The EPOS model with hydrodynamic evolution \cite{Werner:2013tya} does not show this behaviour in the predictions for the yield of muons from heavy-flavour hadron decay.
% (ALICE talk HP2018).
Moreover, measurements of $J/\psi$ relative yields versus $D$-mesons with the same transverse mass could help to elucidate the influence, if any, of hadronisation and of initial-state effects.
...
Also, within the CGC+NRQCD framework \cite{Ma:2018bax}, the relative contribution of each of the channels $^{2S+1}L_J^{[c]}$ with increasing event activity has been calculated. 
$S$, $L$ and $J$ denote spin, orbital, and total angular momentum, and $c$ refers to a color singlet $c=1$ or color octet $c=8$ state.
%
As it is shown in Figure~\ref{fig:fig},
the relative yield of $^3S_1^{[8]}$ to $J/\psi$ production has been found to be larger than the other channels for all $dN_{\rm ch}/d\eta$, and it increases significantly with increasingly rare events. The growth in the contributions of the $^1S_0^{[8]}$  and $^3P_J^{[8]}$ channels are relatively much smaller, which suggests that the LDMEs of these channels could potentially be smaller. 
This may provide a way for reconciling the LDMEs extracted from hadroproduction with the ones from $e^+ e^-$ data.
Moreover, if measurements of $\eta_c$ were possible, one could apply sets of $J/\psi$ LDMEs to $\eta_c$ LDMEs making use of the NRQCD heavy quark spin symmetry, to check compatibility.
That could help to clarify the universality of NRQCD LDMEs.

\begin{figure}[thb]
\hspace{-0.3cm}
\includegraphics[width=0.5\linewidth]{\main/quarkonia/fig/theory/Jpsi-Nch-ALICE-pp-7and13TeV.pdf}
\includegraphics[width=0.5\linewidth]{\main/quarkonia/fig/theory/Jpsi-Nch-pp-NRQCD.pdf}
\vskip -0.2cm
\caption{\label{fig:fig}%(Color online) 
On the left:
Model comparison to ALICE data \cite{Abelev:2012rz,Weber:2017hhm}.
On the right: 
Relative contribution of each of the channels $^{2S+1}L_J^{[c]}$ vs. increasing multiplicity calculated within the CGC+NRQCD framework \cite{Ma:2018bax}.
}
\vskip -0.5cm
\end{figure}
