...
contribute to the different $S$-wave bottomonia, owing to decays of \PGUP{2S} and \PGUP{3S}, as well as the many states of the \PGcb family. In practice, in \pp collisions, up to $30-40$\% of the measured 
...
use of such lQCD-vetted potential, it has been shown~\cite{Krouppa:2017jlg} that predictions are quite sensitive to this choice, as compared to a perturbative potential. Note that the coherence between the observed relative suppression in PbPb and pPb collisions was investigated in \cite{Ferreiro:2018wbd}.
...
This leads to a maximum in the \raa as a function of $\pt$, at around 10\UGeVc for \PGUP{1S}. The current data is not precise enough to confirm or disfavour this feature, but Run~3+4 
...
anisotropies are limited. For the same reason, the elliptic flow of \PGUP{2} could be a factor 2 or more higher~\cite{Du:2017qkv,Bhaduri:2018iwr}, both from the regenerated and primordial
...
Recently, the measurement of $v_2$ of \PJgy in \pPb collisions became available~\cite{Acharya:2017tfn,Sirunyan:2018kiz}, indicating a large -- and completely unexpected -- flow magnitude.
...
\subsubsection{High-multiplicity pp collisions} %E. Ferreiro + AA
...
Specifically, the measurements of the self-normalized yields (the yield divided by the mean yield in minimum bias collisions) as a function of the self-normalized charged-particle multiplicity show an increase which is stronger than linear at the highest multiplicities. % for the central rapidity region.
%
...
%The comparison of the above cited models to ALICE experimental data is shown in Figure~\ref{fig:figEF}.

Runs 3, 4 data at unprecedented high multiplicities %with the increased centre-of-mass energy of 14 TeV in $pp$ collisions and larger luminosities, can allow to reach higher multiplicities, in order to confirm the observed enhancement and its energy independence.
...
%The EPOS model with hydrodynamic evolution \cite{Werner:2013tya} does not show this behaviour in the predictions for the yield of muons from heavy-flavour hadron decay. % (ALICE talk HP2018) ...not clear!
...
%\includegraphics[width=0.5\linewidth]{\main/quarkonia/fig/theory/Jpsi-Nch-pp-NRQCD.pdf} \vskip -0.2cm
%\caption{\label{fig:figEF} On the left: Model comparison to ALICE data \cite{Abelev:2012rz,Weber:2017hhm}. On the right: Relative contribution of each of the channels $^{2S+1}L_J^{[c]}$ vs. increasing multiplicity calculated within the CGC+NRQCD framework \cite{Ma:2018bax}.} \end{figure}
...
%
%As shown in Figure~\ref{fig:figEF},
...
%Moreover, if measurements of $\eta_c$ were possible, one could apply sets of \PJgy LDMEs to $\eta_c$ LDMEs making use of the NRQCD heavy quark spin symmetry, to check compatibility. That could help to clarify the universality of NRQCD LDMEs.
