...
The study of bottomonia with \PbPb data from the Runs 3 and 4 of the LHC can bring further information on the various physics aspects described above.
...
contribute to the different $S$-wave bottomonia, owing to decays of \PGUP{2S} and \PGUP{3S}, as well as the many states of the \PGcb family~\cite{Andronic:2015wma}. In practice, in \pp collisions, up to 30--50\% of the measured 
...
Another striking feature is that excited states are more suppressed than the ground state, the \PGUP{3S} being still unobserved in \PbPb collisions ($\raa(\PGUP{3S})<0.094$ at 95\% confidence
...
is already found between models and data regarding this suppression, within current uncertainties. Figure~\ref{fig:upsi_raa_npart} shows that the projected uncertainty on the \raa of \PGUP{1S} will be much smaller than the current model uncertainties.

Differences exist however in the treatment of the suppression of the bottomonia in the medium.
For instance, the heavy quark potential, a Debye screened potential above the deconfinement temperature, as proposed originally~\cite{Matsui:1986dk}, is one important
...
using \unit[10]{nb}$^{-1}$ of data after the Runs 3-4 are shown as a function of $\pt$ in Fig.~\ref{fig:upsi_raa_pt} and $y$ in Fig.~\ref{fig:upsi_raa_y}, and compared to the expectations from two 
