...
$2.5<y<4$), CMS ($\pT>6.5\UGeVc$, for  $|y|<1.2$, $\pT>3.5\UGeVc$, for  $1.2<|y|<2.4$ ?), and ATLAS ($\pT>7\UGeVc$, for  $|y|<2$).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Charmonia in PbPb collisions} % (Main contributor: Anton Andronic)}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

A remarkable discovery at the LHC was that the suppression of \PJgy is significantly reduced in comparison to lower energies~\cite{} and that $\R_{AA}$ is enhanced towards lower \pT~\cite{Abelev:2013ila,Adam:2016rdg}.
...
Recently, the measurement of a significant elliptic flow coefficient $v_2$ of \PJgy \cite{Acharya:2017tgv} brought further crucial support.
...
\end{center}
 \caption{Prompt \PJgy RAA vs \pT~ ($|y|<2$) at high \pT~\cite{Aaboud:2018quy}}
\end{figure}
...
\end{center}
 \caption{Production ratio $\psi(2S)/\PJgy$ vs. $N_{part}$ for $|y|<0.9$ (left) and $2.5<y<4$ (right). Model predictions for the transport approach and for statistical hadronization are included.}
\end{figure}
