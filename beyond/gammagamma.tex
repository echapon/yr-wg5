...
\end{figure}
%-------------------------------------------------------------------
Figure~\ref{fig:lbyl_alice} shows predictions dedicated for the ALICE and LHCb experiments with photon acceptance in $|\eta^{\mathrm{\gamma}}|<0.9$~(top panel) or $2<\eta^{\mathrm{\gamma}}<4.5$~(bottom panel), respectively, and photon energy $E_{\mathrm{\gamma}}$ > 200~MeV~(ALICE) or $E_{\mathrm{T,\gamma}}$ > 200~MeV~(LHCb) for two systems: Pb--Pb collisions at 5.52~TeV~(left panel) and Ar--Ar collisions at 6.3~TeV~(right panel). Presented results include the effect of the experimental energy resolution~\cite{Acharya:2018yhg,Govorkova:2015vqa}. The black-solid lines depict LO QED fermionic box mechanism.
...
These results suggest that both ALICE and LHCb Collaborations could measure LbyL scattering for $W_{\mathrm{\gamma\gamma}}>$ 2~GeV in Pb--Pb collisions. The cross sections are about two orders of magnitude lower in the Ar--Ar system due to smaller electric charge associated with argon beams.
