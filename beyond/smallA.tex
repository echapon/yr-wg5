...
For example, for \ArAr the expectation for one month of collisions is 1080 $\invnb$ ($p=1.5$).  In order to compare across different collisions species we consider the \textit{nucleon--nucleon} integrated luminosity per month of running, which could be larger by a factor 8--25 (for $p=1.5$--1.9) with respect to \PbPb collisions, i.e.\,one month of \ArAr collisions would be equivalent to $\sim$ 25--80 $\invnb$ of \PbPb collisions. This gain would be the same in all centrality classes (defined in terms of percentiles of the hadronic cross section).
